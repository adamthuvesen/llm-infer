...
* :meth:`logits` — full recompute over the whole sequence, no cache. The known-good
  reference path; left bit-for-bit intact.
...
class QwenModel:
    ...
    def logits(self, token_ids: list[int]) -> torch.Tensor:
        ...
        Full recompute over the whole sequence — no cache. The known-good reference path.
        """
        if not token_ids:
...
def _rope_theta(config: object) -> float:
    ...
    if params is not None:
        ...
        if rope_type != "default":
            raise ValueError(
                f"unsupported rope_type {rope_type!r}; reference check handles 'default' only"
            )
