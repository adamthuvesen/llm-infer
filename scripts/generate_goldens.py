...
decode, and stores the prompt ids + continuation ids. The reference check replays
these goldens
without ever re-running HF.

HF reference is computed by **full-recompute greedy** — one ``forward`` over the whole
sequence per step, ``argmax``, append — *not* ``model.generate``. This matches the
llm-infer unit reference check's full-recompute algorithm class. ``generate`` runs a
...
DTYPE = "float32"  # fp32: greedy tie-breaks near-vanish (see docs/fixture-format.md)
MAX_NEW_TOKENS = 40
...
def _hf_full_recompute_greedy(
    ...
    The same algorithm the llm-infer engine runs, so the only thing the reference check compares
