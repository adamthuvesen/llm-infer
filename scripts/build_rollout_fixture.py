"""Freeze the 8-prompt rlvr-sql GRPO rollout slice as a committed, reproducible fixture.

Phase E's rollout-timing comparison replays rlvr-sql's *actual* GRPO rollout shape, not a
synthetic microbench. One GRPO rollout batch is 8 prompts × G=4 = 32 completions
(``per_device_batch_size=8``, ``num_generations=4`` in rlvr-sql's GrpoHyperparams). The
prompts must be byte-identical to rlvr-sql's eval path, so this builds them through
rlvr-sql's OWN builders — ``build_messages(style='cot')`` + ``serialize_schema`` — then
...
benchmark replays them with no rlvr-sql dependency and no re-tokenization drift.

This runs locally, once. It needs rlvr-sql's builders (pure pydantic) and ``datasets`` for
...
Inputs (override via env): ``RLVR_SQL_ROOT`` (rlvr-sql checkout, read-only),
``TEXT2SQL_SPIDER_ROOT`` (an extracted ``spider_data/`` for ``tables.json`` — rlvr-sql's
...
RLVR_SQL_ROOT = Path(os.environ.get("RLVR_SQL_ROOT", Path.home() / "dev" / "menti" / "rlvr-sql"))
SPIDER_ROOT = Path(
    os.environ.get("TEXT2SQL_SPIDER_ROOT", RLVR_SQL_ROOT / "data" / "spider" / "spider_data")
)

# rlvr-sql GRPO rollout shape (text2sql_rlvr.train.grpo.GrpoHyperparams) — the anchor is grpo-s0.
...
def _load_build_messages(rlvr_root: Path):
    """Load rlvr-sql's ``build_messages`` from prompt.py without importing the eval package."""
    prompt_path = rlvr_root / "src" / "text2sql_rlvr" / "eval" / "prompt.py"
    if not prompt_path.is_file():
        raise FileNotFoundError(f"rlvr-sql prompt builder not found at {prompt_path}")
...
def main() -> None:
    ...
    from text2sql_rlvr.data.artifacts import SPIDER_HF_REPO, SPIDER_HF_REVISION
    from text2sql_rlvr.data.schema import serialize_schema
    from text2sql_rlvr.data.spider import load_examples, load_schemas

    from llm_infer.model.config import MODEL_ID, MODEL_REVISION

    # Load build_messages straight from its file: importing text2sql_rlvr.eval as a package runs
    ...
    if not tables_json.is_file():
        raise FileNotFoundError(
            ...
            "spider_data/ dir (rlvr-sql's data/spider/spider_data by default)."
        )
