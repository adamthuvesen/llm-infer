"""Historical Qwen rollout-timing comparison for llm-rlvr GRPO.

Archived public-baseline evidence for the old Qwen/llm-rlvr hook. Extends the benchmark harness
(``llm_infer/benchmarks/`` runners + report, the same pinned-config Modal pattern) to the *sampled*
rollout workload: one frozen
llm-rlvr GRPO rollout batch (anchor ``grpo-s0``) — 8 Spider-dev prompts × G=4 = 32
...
* ``vllm`` — llm-rlvr's *current* rollout backend, the ceiling (never the thing we beat);
...
Unlike benchmark there is **no** cross-system token-equivalence check: under sampling the engines
use different RNG, so identical tokens are neither expected nor clear to require. The metrics
...
def main(
    ...
    config = {
        ...
        "served_model": {
            ...
            "adapter": "llm-rlvr grpo/grpo-s0 (rank 32), merged bf16",
            "dtype": "bfloat16",
        },
        "workload": {
