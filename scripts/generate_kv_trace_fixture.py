...
Block lifecycle is emitted clearly. A small ``BlockPool`` mirrors the real refcounted
...
class BlockPool:
    """A refcounted free list mirroring ``BlockAllocator`` so block events stay clear.
...
def _preempt(
    ...
    """Evict ``victim`` by recompute: free its KV clearly, keep its tokens, requeue it front."""
...
def build_trace_jsonl() -> str:
    ...
    engine evicts the most-recently-admitted one (LIFO) — freeing its KV clearly and keeping
    ...
    tokens, and the block lifecycle stays clear throughout.
    """
    tb = TraceBuilder()
