"""Merge the rlvr-sql GRPO LoRA adapter into the base model, once, as a served artifact.

Phase E serves a *merged* checkpoint so the engine stays LoRA-free: the rlvr-sql GRPO run
...
The adapter is read-only input on rlvr-sql's Modal volume ``text2sql-runs`` at
...
HF_CACHE = "/hf-cache"
RUNS_MOUNT = "/runs"  # rlvr-sql's text2sql-runs volume (read-only input)
...
# rlvr-sql's training-runs volume (its GRPO adapters). Must already exist — we only read it.
