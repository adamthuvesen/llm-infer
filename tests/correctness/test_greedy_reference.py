"""Reference check: single-request greedy decode, token-for-token vs HuggingFace.

Every attention backend is validated against this. The committed golden fixture holds
HF greedy continuations for the pinned model; here the llm-infer engine decodes the same
prompt ids through the ``torch_naive`` reference backend and must produce identical token ids.
No re-running HF: the goldens are the frozen HF reference (regenerate with
``scripts/generate_goldens.py``).

The model load is module-scoped (one 3B fp32 load for the whole suite) and marked
``slow`` so the default local check stays quick. Run this suite explicitly with
``uv run pytest tests/correctness -q -m slow`` when the full 3B reference check is needed.
"""
...
def test_fixture_pins_the_model() -> None:
    """The fixture must be the pinned Instruct model.

    A mismatch invalidates the reference check.
    """
