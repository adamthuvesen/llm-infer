...
The whole thesis of the engine is token-exactness, so the rule for preemption is the same
same rule as everywhere else: a request that is preempted under KV pressure and later resumed by
...
def test_preemption_actually_fires_under_a_tight_budget() -> None:
    ...
    # Each preemption frees the victim's KV clearly and names the pressure reason.
