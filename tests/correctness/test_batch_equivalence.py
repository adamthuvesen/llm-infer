...
this is the check that the *full* batch the benchmark times is still exactly correct. Same
``torch_naive`` backend (materialized softmax, no fast kernel), so the rule is exact — a
