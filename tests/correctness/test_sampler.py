...
* the fast tensor-level checks — temperature 0 == argmax, top-k / top-p truncate to the
  ...
  rejected loudly;
* the integration rule — the engine driven entirely through ``SamplingParams(temperature=0)``
  (prefill + batched decode) reproduces the committed HF greedy goldens token-for-token. That
  ties the per-row sampling code path back to the checked cached greedy path.

CPU-runnable, fp32 — the same exact rule the greedy reference check holds.
"""
...
def test_invalid_seed_rejected_before_torch_generator() -> None:
    """manual_seed raises on out-of-range ints; reject them with sampler validation first."""
    SamplingParams(seed=-(2**63))
    SamplingParams(seed=2**64 - 1)
    with pytest.raises(ValueError, match="seed must be in"):
        SamplingParams(seed=-(2**63) - 1)
    with pytest.raises(ValueError, match="seed must be in"):
        SamplingParams(seed=2**64)
