"""The rlvr-sql prompt builder, replicated byte-for-byte.

The oracle is only meaningful if it decodes the *exact* prompts rlvr-sql feeds the
model. This is a verbatim copy of rlvr-sql's ``cot`` path — the ``cot`` system
instruction, ``serialize_schema``'s ``CREATE TABLE`` blocks, and ``build_messages``'s
user-content layout — pinned here so the engine has no dependency on the rlvr-sql
...
Source (read-only, rlvr-sql @ Jun 2026):
  src/text2sql_rlvr/data/schema.py :: serialize_schema
  src/text2sql_rlvr/eval/prompt.py :: INSTRUCTION ("cot"), _user_content, build_messages
"""
...
# Verbatim rlvr-sql "cot" system instruction (eval/prompt.py :: INSTRUCTION).
COT_INSTRUCTION = (
...
def serialize_schema(schema: DatabaseSchema) -> str:
    """Verbatim rlvr-sql ``serialize_schema`` (data/schema.py). Deterministic by construction."""
...
def _user_content(schema_text: str, question: str, evidence: str | None = None) -> str:
    """Verbatim rlvr-sql ``_user_content`` (eval/prompt.py)."""
...
def build_cot_messages(
    ...
    """The rlvr-sql ``cot`` chat messages with no few-shot exemplars.
