...
* ``decode_many`` over B requests == ``decode_one`` on each, token-for-token (the unit bar);
* an all-cases batch driven through the engine reproduces the committed goldens (the
  integration bar — the engine now uses ``decode_many`` internally, so this also guards it).

Same ``torch_naive`` backend, fp32 — so the bar is EXACT. A divergence is a batching/position/
gather bug, not FP noise. CPU-runnable, zero GPU spend; the flash backend's batched path is
validated on the A100 by the benchmark's fp32-truth agreement check.
"""
