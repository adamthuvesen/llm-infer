"""Pin the replicated rlvr-sql prompt builder against its exact expected bytes.

``tests/correctness/prompt.py`` is a verbatim copy of rlvr-sql's ``cot`` path (it
must not import rlvr-sql). These tests freeze the exact output so any drift in the
copy breaks the build instead of silently desynchronizing the goldens. The expected
strings below were produced by the real rlvr-sql functions and compared byte-for-byte
