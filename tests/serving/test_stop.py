...
the stop, and an over-limit ``stop`` is a 400. Token ids are already validated
by the reference check; the scripted engine is only a deterministic token source for
the transport-level stop contract.
"""
