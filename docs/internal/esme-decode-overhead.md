...
* **Kernel-launch dispatch is the wall.** ~2,100 `cudaLaunchKernel` calls per step;
  ...
  math. The paged-KV bookkeeping is not the source: `rms_norm` runs 121 times/step (30 layers x
...
* **GPU "busy" is itself launch-shaped.** 19 ms/step of GPU time across ~2,100 tiny kernels
  ...
  overhead rather than waiting on one big sync.
* **Block-table Python scaled with batch.** At batch 128 the cProfile top was
  `physical_slot` (594k calls), `prepare_write` (237k), and `capacity` (831k). `write_many`
...
* **Per-token host syncs existed but were cheap.** One `.cpu().tolist()` EOS check per step;
  since CPU dispatch was already behind the GPU, removing that sync alone could not move the wall.
...
* Change 1: below 10% alone (noise-level), with near-zero complexity. Kept.
...
The remaining decode wall after these changes was kernel-launch dispatch from ~1,850 tiny
