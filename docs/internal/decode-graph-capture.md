...
* **Piecewise: attention and the paged-KV write stay eager.** Per step the runner replays
  ...
  layer, then final norm+lm-head+soft-cap — with the flash-attn varlen call, the packed
  history gather, and `write_rows` running eagerly between segments. The ragged KV history
...
1140 MHz, 400 W — a slower host than the 2026-07-02 overhead-doc container; compare within
...
batch-independent while useful GPU math scales with batch — exactly the expected shape.
