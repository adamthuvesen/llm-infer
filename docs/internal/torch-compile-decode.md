...
CUDA-graph decode runner but left it unwired. This pass (A) makes the captured path what
...
(118.7 vs 155.2 tok/s; manual graphs: 425.0). The gap comes from host-side per-step framework
overhead. The compiled runner is now an opt-in measurement path.
...
| `scripts/modal_esme_three_way.py` | ON for the llm_infer row | none | `DEFAULT_CAPTURE_SIZES` (covers headline batch 64) |
| `scripts/modal_esme_batch_curve.py` | ON for the llm_infer rows | none | `(1..128, 256)` so batch 256 replays instead of falling back |

Batches above the largest bucket (and windows past the pinned RoPE rows) fall back to the
eager planned window per window. That path is correct and slower. Capture time on this container:
...
`model.decode_graphs` slot as the manual runner: buckets, padding, eager fallbacks. The
segment math goes to Dynamo/Inductor instead of hand-captured graphs:

* **Opaque attention boundary.** `llm_infer::paged_decode_attention` is a mutable custom op
  (`Tensor(a!) out`, `register_fake` for tracing, `cudagraph_unsafe` tag) that runs the
  eager per-layer work: paged-KV `write_rows`, ragged history gather, GQA expansion,
...
(Triton builds its launcher stubs with `cc` at runtime, so `build-essential` was added), and
...
| --- | ---: | ---: | ---: | ---: |
| eager-window | 12,986 | 0 | 101.6 ms | none |
...
runner's steady-state step is 31 `graph.replay()` calls plus the eager attention work:
...
* **Measured only:** `CompiledDecodeRunner` stays as an opt-in measurement path.
  Nothing constructs it by default, and no serving entry point references it. It is kept
