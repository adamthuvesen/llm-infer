...
flowchart TB
    ...
    subgraph serving [serving/]
        ...
        Trace["TraceRecorder optional JSONL events"]
    end
...
| [`tracing.py`](../llm_infer/tracing.py) | Opt-in schema-versioned engine events for JSONL replay | Cache-block lifecycle events; those need request-aware cache hooks first |
| [`visualizer/`](../visualizer/) | Static local schema-v2 trace viewer, parser smoke tests, playback UI | Network assets, fake block allocation/free events |
...
- RoPE positions are **per-request sequence length**, never batch row index — documented
  ...
  breaks paged decode.
- Scheduler **reserves** worst-case blocks but **allocates lazily**; prefix caching can share
...
2. For each running request without `prefilled`: cache one prompt chunk, bounded by
   `InferenceEngine(prefill_chunk_size=...)`. Requests with the same `prefix_group_id` and
   identical prompt can share refcounted prompt blocks. When the full prompt is cached,
   sampler picks the first token.
...
### Flow B3 — Opt-in trace emission for KV-cache-theater

Implemented by [`TraceRecorder`](../llm_infer/tracing.py) and
`InferenceEngine(trace=recorder)`. Tracing is off by default and records only real
`InferenceEngine` events; tests cover actual engine execution in
[`test_tracing.py`](../tests/serving/test_tracing.py).

Schema version **2** event names:
...
to make the range explicit. `sequence` is the replay order; `step` is the engine loop tick.

The local viewer lives in [`visualizer/`](../visualizer/) and consumes schema-v2 JSONL directly.
The committed fixture at [`docs/assets/kv_trace_schema_v2.jsonl`](assets/kv_trace_schema_v2.jsonl)
is regenerated by [`scripts/generate_kv_trace_fixture.py`](../scripts/generate_kv_trace_fixture.py),
which drives the real `InferenceEngine(trace=...)` path with a deterministic clock.

Block lifecycle events are intentionally absent. The clean follow-up hook is lower than the
engine request state: add request-aware observation around `BlockTable.reserve()` /
`BlockTable.free()` and copy-on-write in `PagedKVCache.prepare_write()`, then emit
`block_allocated` / `block_freed` from that layer.
...
**Prefix caching.** Requests in a declared sibling group with identical prompts can share full
prompt blocks by refcount. Generation into a shared partial prompt block uses copy-on-write in
[`PagedKVCache.prepare_write`](../llm_infer/kv_cache/paged_kv_cache.py).
...
**Trace contract.** `TraceRecorder.to_jsonl()` is the visualizer handoff artifact. Consumers
must replay by `sequence`, treat optional fields as absent rather than zero, and draw prefill
from chunk ranges rather than assuming one prefill event per request.
...
- How do prefix block sharing and chunked prefill interact with scheduler reservation in
