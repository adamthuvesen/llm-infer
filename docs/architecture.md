...
The local viewer lives in [`visualizer/`](../visualizer/) and consumes schema-v3 JSONL directly.
The committed fixture at [`docs/assets/kv_trace_schema_v3.jsonl`](assets/kv_trace_schema_v3.jsonl)
...
Block lifecycle is emitted from the physical boundary, not faked from request state. The
allocator (`BlockAllocator`) is the single owner of the free pool, so it fires an observer
after every change: `block_allocated` when ids leave the pool (copy-on-write in
`PagedKVCache.prepare_write()` included) and `block_freed` only for the ids that truly return
(refcount-0). A prefix-shared block retained by a forked sibling never re-allocates and is
freed once, by its last owner. Each event carries `request_id`, `block_count`, `block_ids`, and
the post-change pool `pool_used` / `pool_free` totals.
