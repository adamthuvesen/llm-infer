# Keeping the GPU Busy — llm-rlvr Rollout Timing

This is the differentiator: the one number that makes `llm-infer` an *llm-rlvr rollout
...
**real** thing it exists to serve — one frozen llm-rlvr GRPO rollout batch — and times it
against vLLM (llm-rlvr's current rollout backend, the ceiling) and naive HF (the floor).
...
One GRPO rollout batch from llm-rlvr's own `GrpoHyperparams`, anchored to **`grpo-s0`** (the
shipped main result):
...
The prompts are **byte-identical to llm-rlvr's eval path**: built through its own
...
llm-rlvr import and no re-tokenization drift. The chosen dev rows span six distinct schemas
...
llm-rlvr ships its policy as a **rank-32 PEFT LoRA adapter** over
...
llm-rlvr's GRPO run was bf16), it is the dtype the engine's greedy oracle already validates,
...
| row | what it is | role |
| --- | --- | --- |
| `vllm` | vLLM offline generate, prefix caching off, flags pinned, bf16 merged weights. | **The ceiling** — llm-rlvr's *current* rollout backend. Never the thing we beat. |
...
Run `2026-06-21`. Served weights: `Qwen/Qwen2.5-Coder-3B-Instruct` @ `488639f1` + llm-rlvr
...
lower $/1k** ($1.00 vs $1.85) — the same kind of measured speedup, now on llm-rlvr's actual
...
What this buys the claim: llm-rlvr's GRPO loop spends most of its wall-clock in rollout
generation. This is the hook that connects `llm_infer`'s throughput to that loop's economics —
$/1k rollouts at a pinned GPU price, on a frozen, reproducible batch built from llm-rlvr's own
...
- **Served model:** base `Qwen/Qwen2.5-Coder-3B-Instruct` @ `488639f1ff808d1d3d0ba301aef8c11461451ec5`
  merged with llm-rlvr `grpo/grpo-s0` (PEFT LoRA, rank 32, α 64, all-linear), `merge_and_unload`,
  saved **bf16**.
- **Workload:** `tests/fixtures/rollout_grpo_s0_spider_dev.json` — Spider dev (HF `xlangai/spider`
  ...
  through llm-rlvr `build_messages(cot)` + `serialize_schema`; 8 prompts × G=4 = 32 completions,
...
# (re)build the frozen 8-prompt fixture from llm-rlvr's builders (local, no GPU)
